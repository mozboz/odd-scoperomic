...
autoComplete = function(completeThis, context, exact) {
	...
	var searchFor = completeThis.replace(/[\-\[\]{}()*+?.,\\\^$|#\s]/g, "\\$&");
...
/**
 * Checks if a object is a oid string.
 * @param stringToCheck the object to check
 * @returns {Boolean} if the object is a oid or not.
 */
isOid = function(stringToCheck) {
	if (typeof stringToCheck == "undefined")
		throw "The stringToCheck parameter is not specified";
	if (typeof stringToCheck != "string")
		return false;

	var groups = stringToCheck.match("([0-9a-fA-F-]*)#(\\w*)");
...
/**
 * Takes a object and loads all key and value objects for it. 
 * Then it creates a new object which only contains the names of the
 * key and value objects for display purposes.
 * @param obj The object
 * @returns the display object
 */
createDisplayObject = function(obj) {
	...
	for (var f in obj) {
		...
		if (isSystemField(f) || !isOid(f)) {
			// simply copy
			displayObject[f] = obj[f];
		} else {
			// get the key and value objects
			var keyOidParts = parseOid(f);
			var keyObj = loadObject(keyOidParts.id, keyOidParts.rev);
			var valObj = obj[f];
			
			if (typeof valObj != "undefined" 
				&& valObj != null
				&& isOid(valObj)) {
				var valOidParts = parseOid(valObj);
				valObj = loadObject(valOidParts.id, valOidParts.rev);
			}
			
			//@todo: respects only the name for the moment. This has to change.
			displayObject[keyObj.name] = valObj == null 
													? undefined
													: valObj.name;
		}
	}
